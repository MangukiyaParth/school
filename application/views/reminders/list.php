	...
	<section class="container-fluid">
		<div class="search-div row justify-content-center" style="margin-bottom: 25px; margin-top: 25px;">
			<div class="col-sm-offset-2 col-sm-3">
				...
				<input type="text" name="search" id="search" class="from-control" />
			</div>
			<div class="col-sm-1">
				<button class="btn btn-primary" id="apply_filter">Apply</button>
			</div>
			<div class="col-sm-offset-5 col-sm-1">
				<a class="btn btn-success" href="<?php echo base_url(); ?>reminders/add-reminder/0">Add</a>
			</div>
		</div>
		<div class="data-div">
...
<script type="text/javascript">
	...
	function editData(id){
		window.location = BASE_URL+"reminders/add-reminder/"+id;
	}
	function deleteData(id){
		if (confirm("Are you sure?")) {
			window.location = BASE_URL+"reminders/delete-reminder/"+id;
		}
		return false;
	}
	...
	function __onLoadStudentData(data) {
		$.ajax({
			type:"POST",
			url :BASE_URL+"reminders/reminder-list/"+start,
			async:false,
			data:data,
	...
	function setTableFormatData(start, data, resultArr) {
		...
		for (var i = 0; i <= listArr.length; i++) {
			if (i == listArr.length) {
				...
				if(listArr[i].reminder_for == 1) { var reminder_for = 'Students' } else { var reminder_for = 'Staff' }
				tableData += '<tr class="rowId_'+listArr[i].id+'">';
					tableData += '<td>'+listArr[i].title+'</td>';
					tableData += '<td>'+listArr[i].valid_from+'-'+listArr[i].valid_to+'</td>';
					tableData += '<td>'+ reminder_for +'</td>';
					tableData += '<td>'+listArr[i].subject+'</td>';
					tableData += '<td>'+listArr[i].insert_at+'</td>';
					tableData += '<td><button class="btn btn-primary" onclick="editData('+listArr[i].id+')">Edit</button><button class="btn btn-danger ms-3" onclick="deleteData('+listArr[i].id+')">Delete</button></td>';
				tableData += '</tr>';
			}
		}
